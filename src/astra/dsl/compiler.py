...
def compile_mission(dsl: MissionDSL, ephemeris: EphemerisEngine | None = None) -> CompiledMission:
    ...
    # Precedence: multi_body_trajectory overrides trajectory
    traj_schema = (
        dsl.multi_body_trajectory
        if dsl.multi_body_trajectory is not None
        else dsl.trajectory
    )

    # Bodies
    origin = CelestialBody[traj_schema.origin.body.upper()]
    destination = CelestialBody[traj_schema.destination.body.upper()]
    ...
    h_park = get_default_parking_altitude(traj_schema.origin.body)
    if traj_schema.origin.orbit is not None:
        if traj_schema.origin.orbit.altitude_km is not None:
            h_park = traj_schema.origin.orbit.altitude_km
        elif traj_schema.origin.orbit.periapsis_km is not None:
            h_park = traj_schema.origin.orbit.periapsis_km

    h_cap = get_default_parking_altitude(traj_schema.destination.body)
    capture_apoapsis_km = None
    if traj_schema.destination.orbit is not None:
        orbit = traj_schema.destination.orbit
        if traj_schema.destination.orbit.altitude_km is not None:
            h_cap = traj_schema.destination.orbit.altitude_km
        elif traj_schema.destination.orbit.periapsis_km is not None:
            h_cap = traj_schema.destination.orbit.periapsis_km
    ...
    if dsl.multi_body_trajectory is not None:
        compiled.flyby_sequence = [
            ...
            for fb in dsl.multi_body_trajectory.flyby_sequence
        ]
