...
def test_pinn_warmstart_quality():
    ...
    r_pinn = optimize_mission_pinn_accelerated(
        mission, kernel, n_trials=1000, time_limit=120.0, seed=42,
        pinn_train_samples=300, pinn_epochs=30)
    ...
    assert dv_pinn <= dv_std * 1.05, (
        f"PINN {dv_pinn:.3f} is >5% worse than standard {dv_std:.3f}")
